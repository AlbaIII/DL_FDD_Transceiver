...
from C3_Model_define import *  #在此处切换使用Hybird系统或Digital系统
...
# all_feedback_bit =    [32 , 32, 32, 32, 32, 32 , 32 , 32]
...
h_imag = np.float32(np.transpose(h5py.File(fname_h_imag)['h_imag'])) #维度[N,subcarrier,UE_antenna,BS_antenna]
...
h_concat_scaled = np.concatenate((  np.expand_dims(h_real,-3) / norm_factor, np.expand_dims(h_imag,-3) / norm_factor), axis=-3)  # 将实部和虚部拼接起来，维度[N,subcarrier, 2 ,UE_antenna, BS_antenna]
...
h_train = torch.utils.data.TensorDataset(h_train) 
h_val = torch.utils.data.TensorDataset(h_val)
h_test = torch.utils.data.TensorDataset(h_test)
...
for tap,noise_power_dBm in enumerate(all_noise_power_dBm):
    ...
    feedback_bit = all_feedback_bit[tap]

    #模型构建与训练

    #Hybrid系统模型
    model = Beamformer_Network( BS_antenna=BS_antenna, UE_antenna=UE_antenna, BS_RF_chain=BS_RF_chain, UE_RF_chain=UE_RF_chain,
                                DATA_stream=DATA_stream,Subcarrier=Subcarrier, Subcarrier_group=Subcarrier_group, feedback_bit=feedback_bit,
                                noise_power=noise_power, norm_factor=norm_factor ).cuda()




    learnable_codebook_opt = optim.Adam(model.parameters(), lr=0.0003, betas=(0.9, 0.999),amsgrad=False)
    lambda1 = lambda epoch: 0.9995 ** epoch
    scheduler = torch.optim.lr_scheduler.LambdaLR(learnable_codebook_opt, lr_lambda=lambda1)      

    print('{}Dataset_{}bit_{}dBm_noise.pt'.format(dataset_name,int(32),noise_power_dBm))
    train_loss_hist, val_loss_hist = Model_fit(model=model, h_train=h_train, h_val=h_val,  opt=learnable_codebook_opt, scheduler = scheduler,  EPOCHS=nepoch,
                                                dataset_name=dataset_name,bit = 32, noise_power_dBm=noise_power_dBm)
