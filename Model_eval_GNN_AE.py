...
from GNN_Model_define_AE import *  #在此处切换使用Hybird系统或Digital系统
...
# all_feedback_bit = [256,256,256,256,256,256,256,256]
# all_feedback_bit = [128,128,128,128,128,128,128,128]

#改变传输bit数量
all_noise_power_dBm = [-94,-94,-94,-94,-94,-94,-94]
all_feedback_bit = [4,8,16,32,64,128,256]
...
for tap,noise_power_dBm in enumerate(all_noise_power_dBm):
    ...
    model = Beamformer_Network( BS_antenna=BS_antenna, UE_antenna=UE_antenna, RF_chain=RF_chain, 
                                ...
                                Feedback_feature=Feedback_feature,  Feedback_bit = Feedback_bit,
                                noise_power=noise_power,estimate_noise_power=estimate_noise_power, norm_factor=norm_factor ).to(device)



    learnable_model_savefname = '/home/yangjunyi/NAIC/Saved Models/GNNAE_{}Dataset_{}Pilot_{}bit_{}dBm_noise.pt'.format(dataset_name, Pilot_num,Feedback_bit,noise_power_dBm)
    model.load_state_dict(torch.load(learnable_model_savefname))

    print('{}Dataset_{}Pilot_{}bit_{}dBm_noise.pt'.format(dataset_name, Pilot_num,Feedback_bit,noise_power_dBm))
