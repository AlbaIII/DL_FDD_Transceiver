...
for tap,noise_power_dBm in enumerate(all_noise_power_dBm):
    ...
    for batch_idx,(h_real,h_imag) in enumerate(h_val):
        h_complex = (h_real + 1j*h_imag).cuda()
        ...
        for i in range(0,int(Subcarrier/Subcarrier_gap)): #第i个发送pilot的子载波
            ...
            #添加噪声
            noise_real = (torch.normal(0,1, size=Y_vector.size()) * torch.sqrt(torch.tensor([noise_power/2]))).float().cuda()
            noise_imag = (torch.normal(0,1, size=Y_vector.size()) * torch.sqrt(torch.tensor([noise_power/2]))).float().cuda()
            Y_vector = Y_vector + noise_real +1j*noise_imag

            #估计信道
            R_h_h = torch.matmul(h_vector,conj_T(h_vector)) #维度[batchsize , 1 ,  BS_antenna×UE_antenna,BS_antenna×UE_antenna ]
            XH_Y = torch.matmul(conj_T(X),Y_vector) #维度[batchsize , 1 ,  BS_anteena*UE_antenna,1 ]
            temp1 = torch.matmul(XH_X,R_h_h)
            temp2 = torch.matmul(XH_X,R_h_h) + noise_power*torch.eye(BS_antenna*UE_antenna,BS_antenna*UE_antenna).cuda()
            temp = torch.linalg.pinv(torch.matmul(XH_X,R_h_h) + noise_power*torch.eye(BS_antenna*UE_antenna,BS_antenna*UE_antenna).cuda()) #维度[batchsize , 1 ,  BS_anteena*UE_antenna,BS_anteena*UE_antenna ]
            h_LMMSE_vector = torch.matmul(R_h_h,torch.matmul(temp,XH_Y )) #维度[batchsize , 1 ,  BS_anteena*UE_antenna,1 ]
            

            h_hat= anti_vector(h_LMMSE_vector,UE_antenna,BS_antenna) * torch.ones(1,Subcarrier_gap,1,1).cuda() #维度[batchsize, Subcarrier_gap  , UE_antenna,BS_antenna]
    ...
    print('LMMSES rate = {:.8f}'.format(RATE))
